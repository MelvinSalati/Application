...
const MCHReports = () => {
  ...
  const getReport = () => {
    if (startDate === "" || endDate === "") {
      ...
      if (startD < endD) {
        ...
        window.open(
          `https://reports.v2.smart-umodzi.com/report.php?report_id=${reportID}&from=${startDate}&to=${endDate}&hmis=${hmis}&department=2`
        );
      } else if (startD === endD) {
        setReportModal(false);
        window.open(
  ...
  const listOfReports = [
    {
      SN: "5",
      title: "Expected pregnant mothers",
      cat: "Appointments",
      desc:
    ...
    {
      SN: "6",
      desc: "Expeceted apointments baby",
      title: "Baby mother pair apppointments",
      cat: "Appointments.",
    },
    // {
    //   SN: "4",
